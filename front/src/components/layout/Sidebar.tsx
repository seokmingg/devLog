    ...
    <aside className={styles.sidebar}>
      ...
      <nav className={styles.nav} aria-label="주 메뉴">
        {navItems.map(([icon, label], index) => (
          label === '내 프로필' ? <Link key={label} className={styles.item} to="/login"><span className={styles.icon}>{icon}</span><span>{label}</span></Link> : <a key={label} className={`${styles.item} ${index === 0 ? styles.active : ''}`} href={index === 0 ? '/' : `#${label}`}>
